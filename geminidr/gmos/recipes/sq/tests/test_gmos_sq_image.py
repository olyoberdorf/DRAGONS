...
def config(request, path_to_inputs, path_to_outputs):
    """
    Sets up and tear down tests for GMOS SQ Image using a commong workflow.
    ...
    Returns
    -------
    ConfigTestsGmosImaging
        An object that contains references to the `.input_dir` and to the
        the calibration manager that should persist along the class.
    """

    # Set up ---
    dataset_dir, binning, obs_class = request.param

    config_for_tests = ConfigTestsGmosImaging(
        dataset_dir, binning, obs_class, path_to_inputs, path_to_outputs)

    yield config_for_tests

    # Tear down ---
    if request.session.testsfailed == 0:
        shutil.rmtree(config_for_tests.output_dir)

    del config_for_tests


class ConfigTestsGmosImaging:
    """
...
class TestGmosReduceImaging:
    """
    Collection of tests that will run on every `dataset_dir`.
    """
